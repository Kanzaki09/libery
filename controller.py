...
def save_book():
    title = st.session_state.get("book_add_title", "")
    author = st.session_state.get("book_add_author", "")
...
def create_user(username: str, password: str, role: str, is_active: bool = True):
    ...
    # 🔧 FIX: รองรับกรณี model ไม่มี is_username_exists
    if username:
        if hasattr(model, "is_username_exists"):
            exists = model.is_username_exists(username)
        elif hasattr(model, "get_user_auth_row"):
            exists = model.get_user_auth_row(username) is not None
        else:
            exists = False

        if exists:
            errors.append(f"ชื่อผู้ใช้ **{username}** มีอยู่แล้ว")
